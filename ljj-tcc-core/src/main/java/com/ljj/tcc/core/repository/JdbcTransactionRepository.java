...
public class JdbcTransactionRepository extends CachableTransactionRepository {
    ...
    protected int doDelete(Transaction transaction) {
        ...
        try {
            ...
            StringBuilder builder = new StringBuilder();
            builder.append("DELETE FROM " + getTableName() + " WHERE GLOBAL_TX_ID = ? AND BRANCH_QUALIFIER = ?");
    ...
    protected List<Transaction> doFindAllUnmodifiedSince(LocalDateTime date) {
        ...
        try {
            ...
            builder.append(
                "SELECT GLOBAL_TX_ID, BRANCH_QUALIFIER, CONTENT,STATUS,TRANSACTION_TYPE,CREATE_TIME,LAST_UPDATE_TIME,RETRIED_COUNT,VERSION");
            ...
            stmt.setTimestamp(1, new Timestamp(date.getgetTime()));
    ...
    protected void constructTransactions(ResultSet resultSet, List<Transaction> transactions) throws SQLException {
        while (resultSet.next()) {
            ...
            transaction.changeStatus(TransactionPhase.valueOf(resultSet.getInt(4)));
    ...
    private String getTableName() {
        return StringUtils.isNotEmpty(tbSuffix) ? "" + tbSuffix : "TCC_TRANSACTION";
    }
}
